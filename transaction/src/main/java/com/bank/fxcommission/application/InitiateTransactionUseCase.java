...
import com.bank.fxcommission.shared.account.Account;
import com.bank.fxcommission.shared.account.AccountFacade;
import com.bank.fxcommission.shared.currency.CurrencyRateFacade;
...
@Service
public class InitiateTransactionUseCase
    implements UseCase<InitiateTransactionUseCaseDto, Transaction> {
  ...
  public Transaction execute(InitiateTransactionUseCaseDto dto) {
    ...
    if (!sourceAccount.isBelongsToCustomer(dto.customerId())) {
      throw new IllegalArgumentException(
          "Account with iban "
              ...
              + " not belongs to customer with id "
              + dto.customerId());
    }
    ...
    if (!quoteAccount.isActive()) {
      throw new IllegalArgumentException(
          "Account with iban " + dto.destinationAccountIban() + " not active");
    }

    BigDecimal baseToReferenceRate =
        this.currencyRateFacade
            .calculateRate(
                sourceAccount.getCurrency(), this.currencyRateFacade.getReferenceCurrency())
    ...
    if (sourceAccount.getCurrency().equals(quoteAccount.getCurrency())) {
      ...
      baseToQuoteRate =
          this.currencyRateFacade
              .calculateRate(sourceAccount.getCurrency(), quoteAccount.getCurrency())
              .setScale(10, RoundingMode.HALF_UP);
    }
